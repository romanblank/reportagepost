...
export const ru = {
  meta: {
    title: 'Репортаж Пост',
    description:
      'Сообщество репортажных фотографов: портфолио, признание цеха и прямые заявки на съёмку событий.',
  },
  country: {
    title: 'Города',
  ...
  pro: {
    kicker: 'Подписка',
    navLabel: 'Подписка',
    title: 'Инструменты мастерской и участие в цехе',
    lead: 'Подписка — не место над коллегами (каталог ранжируется по качеству, одинаково для всех), а полный инструментарий мастерской, участие в «Признании» и поддержка общего цеха. Первые авторы становятся основателями с ценой, закреплённой навсегда.',
    planFree: 'Базовый',
    planPrime: 'Prime',
    planElite: 'Elite',
    tierName: { PRIME: 'Prime', ELITE: 'Elite' } as Record<string, string>,
    free: 'Бесплатно',
    perMonth: 'в месяц',
    perYear: 'в год',
    ...
    betaNote: 'В закрытой бете первые авторы получают подписку бесплатно на время беты — с переходом на цену основателя. Приём оплаты (Т-Касса) активируется к публичному запуску.',
    features: {
      page: 'Публичная страница и профиль в каталоге города',
      inquiries: 'Приём заявок от заказчиков',
      portfolioBasic: 'Портфолио до 20 кадров',
      portfolioUnlimited: 'Портфолио без ограничений и фотоистории',
      richProfile: 'Расширенная страница: пакеты цен, FAQ, оборудование, команда',
      recognition: 'Бейдж и участие в «Признании» — витрине лучших цеха',
      analytics: 'Статистика просмотров и сохранений',
      fastReview: 'Приоритетное рассмотрение изменений',
      recommended: 'Ротация в «Рекомендуемых» и приоритет в редподборках',
      analyticsPlus: 'Расширенная аналитика: кто смотрел и сохранял, тренды',
      earlyAccess: 'Ранний доступ к новым функциям и персональный онбординг',
  ...
  landing: {
    ...
    heroTitle: 'Найти фотографа рядом со мной',
    heroLead: 'Репортажная съёмка событий: конференции, концерты, спорт, корпоративы, частные праздники. Выберите город — сравните портфолио, цены и свободные даты. Прямой контакт без комиссии.',
    heroSearchCity: 'Ваш город',
    heroSearchCta: 'Найти',
    heroPhotographerNudge: 'Вы фотограф?',
    heroPhotographerCta: 'Создать профиль',
    closedNote: 'Открытая регистрация для фотографов. Готовимся к публичному запуску.',
    recentWork: 'Свежие работы',
    registerCta: 'Создать профиль',
    loginCta: 'Войти',
    forPhotographers: 'Фотографам',
    forPhotographersText: 'Профессиональная витрина, признание цеха и целевые заявки — вместо разрозненных соцсетей и сарафана.',
    forClients: 'Заказчикам',
    ...
    step1Text: 'Портфолио, жанры, города и выезды, свободные даты, цены — витрина уровня цеха за один вечер.',
    step2Title: 'Пройдите отбор редакции',
    step2Text: 'Каждую страницу смотрит редакция — в каталог попадают работы, держащие планку. Это и делает признание настоящим.',
    step3Title: 'Получайте заявки',
    step3Text: 'Заказчики находят вас по городу и жанру и пишут напрямую. Сделка — без комиссии платформы.',
    // Отстройка
    whyTitle: 'Почему не соцсети и не биржа услуг',
    whyText: 'На бирже вы — «услуга» в одном ряду с сантехником; в соцсетях работы тонут в ленте. Репортаж Пост — профильная витрина репортажной фотографии: только цех, только событийная съёмка, отбор редакции и цеховое признание вместо случайных отзывов.',
  },
  catalog: {
  ...
  profile: {
    ...
    verified: 'Проверен',
    verifiedHint: 'Личность и работы подтверждены Репортаж Пост',
  ...
  adminPhotographers: {
    ...
    grantPrime: 'Выдать Prime',
    grantElite: 'Выдать Elite',
  ...
  cabinet: {
    ...
    proLockedLead: 'С подпиской открывается:',
    proRequestCta: 'Запросить подписку',
    ...
    proTeaser: (saves: number, reviews: number) =>
      `Ваш профиль замечают: ${saves} в избранном, ${reviews} отзывов. Подписка откроет статистику и продвинет вас.`,
    upsellElite: 'Elite — больше участия и расширенная аналитика →',
    statsTitle: 'Статистика',
    statViews: 'Просмотров',
    statFollowers: 'Подписчиков',
    statLikes: 'Лайков',
    statTrend30d: (v: number, s: number) => `+${v} просмотров и +${s} сохранений за 30 дней`,
  ...
  auth: {
    ...
    registerLeadPhotographer: 'Профиль фотографа: витрина работ, признание цеха и прямые заявки.',
