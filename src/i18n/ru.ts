...
export const ru = {
  meta: {
    title: 'Reportage Post — репортаж как ремесло',
    description:
  ...
  pro: {
    kicker: 'Reportage Post PRO',
    title: 'Статус в цехе и место, которое замечают',
    lead: 'PRO — это бейдж мастера, участие в «Признании», портфолио без границ и приоритет в каталоге. Первые авторы становятся основателями платформы с ценой, закреплённой навсегда. Бесплатный тариф остаётся всегда.',
    planFree: 'Базовый',
    planPro: 'PRO',
    free: 'Бесплатно',
    perMonth: 'в месяц',
    ...
    ctaBecomePro: 'Перейти на PRO',
    ctaInCabinet: 'Управлять подпиской',
    compareTitle: 'Что входит',
    betaNote: 'В закрытой бете первые авторы получают PRO бесплатно на время беты — с переходом на цену основателя. Приём оплаты (Т-Касса) активируется к публичному запуску.',
    features: {
      ...
      priority: 'Приоритет в каталоге и результатах поиска',
      portfolioUnlimited: 'Портфолио без ограничений и фотоистории',
      recognition: 'Бейдж PRO и участие в «Признании» — витрине лучших цеха',
  ...
  landing: {
    ...
    whyText: 'На бирже вы — «услуга» в одном ряду с сантехником; в соцсетях работы тонут в ленте. Reportage Post — профильная витрина репортажной фотографии: только цех, только событийная съёмка, отбор редакции и цеховое признание вместо случайных отзывов.',
  },
  catalog: {
  ...
  profile: {
    ...
    verified: 'Проверен',
    verifiedHint: 'Личность и работы подтверждены Reportage Post',
  ...
  lifecycle: {
    approvedSubject: 'Вы приняты в Reportage Post',
  ...
  auth: {
    registerTitle: 'Присоединиться к Reportage Post',
