...
import { followingOf } from '@/lib/follow-lists';
import { FollowList } from '@/components/FollowList';

// Публичный список подписок фотографа (паритет MyWed). Лезет в БД →
...
export async function generateMetadata(props: { params: Promise<{ username: string }> }): Promise<Metadata> {
  ...
  if (!profile) return {};
  return { title: `${ru.followList.followingTitle} — ${profile.user.firstName} ${profile.user.lastName}` };
}
...
export default async function FollowingPage(props: { params: Promise<{ username: string }> }) {
  ...
  const entries = await followingOf(profile.userId);
  return (
    <main className="mx-auto w-full max-w-xl flex-1 px-4 py-6 sm:py-10">
      ...
      <h1 className="t-h1 mt-3">{ru.followList.followingTitle}</h1>
