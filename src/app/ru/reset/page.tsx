...
function ResetForm() {
  ...
  return (
    <AuthScene>
        ...
        {done ? (
          <>
            <p className="mt-4 t-small muted">{ru.auth.pwreset.resetDone}</p>
            <Link href="/ru/login" className="btn btn-accent mt-5">{ru.auth.pwreset.backToLogin}</Link>
          </>
        ) : !token ? (
          <p className="mt-4 t-small text-danger">{ru.auth.pwreset.resetInvalid}</p>
        ) : (
          <form onSubmit={onSubmit} className="mt-6 flex flex-col gap-4">
            <p className="t-small muted">{ru.auth.pwreset.resetLead}</p>
            <div>
