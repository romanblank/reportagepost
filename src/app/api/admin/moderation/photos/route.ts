  ...
  return handleRoute(async () => {
    ...
    for (const id of ids) {
      try {
        ...
        // «Кадр уже решён» приходит как false, НЕ как исключение — сюда
        // попадают только настоящие сбои (БД, хранилище), и молчать о них
        // значит лишить и админа, и лог шанса отличить «нечего делать» от
        // «сломалось, повтори» (аудит 2026-08-16)
        console.error('[admin] photo decision failed:', id, e);
        failed.push(id);
      }
    }
