...
// Потолок бюджета НА СЪЁМКУ (не за час — семантика едина с заявками и каталогом,
// аудит 2026-07-26). 500 000 ₽ с запасом; гарантирует, что budgetRub*100 влезает
// в Int (int4). Подписи в UI приведены к этой семантике (аудит 2026-07-31).
...
const CATEGORY_KEYWORDS: Record<string, string[]> = {
  ...
  // Свадьбы намеренно НЕ ловим: платформа про репортаж событий, свадебная
  // съёмка — чужая территория (MyWed), и лезть в неё мы не собираемся.
  // Запрос со «свадьбой» лучше оставить без подбора, чем выдать не тех авторов.
  'private-events': ['юбилей', 'день рожден', 'днюх', 'частн', 'вечеринк', 'крестин', 'выпускн', 'семейн'],
  'street-city': ['улич', 'стрит', 'город и', 'репортаж с улиц', 'прогулк по город'],
  // Новые жанры (партнёр 2026-08-18)
  news: ['новост', 'пресс', 'сми', 'редакцион', 'журналист'],
  documentary: ['документал', 'докупроект', 'быт', 'жизнь как есть'],
  theatre: ['театр', 'спектакл', 'премьер', 'репетиц', 'балет', 'опер'],
  'business-portrait': ['делов портрет', 'бизнес-портрет', 'портрет руководител', 'команда для сайта', 'хедшот', 'headshot'],
  industrial: ['завод', 'производств', 'промышлен', 'цех', 'стройк', 'индустри'],
};
...
const LLM_SYSTEM =
  'Ты помощник каталога репортажных фотографов. Извлеки из запроса строго JSON вида ' +
  `{"city":"<город или null>","category":"<${CATEGORIES.map((c) => c.nameRu.toLowerCase()).join('|')}|null>","budgetRub":<число или null>}. ` +
