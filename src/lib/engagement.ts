...
/**
 * Пометка «пересчитать рейтинг» после лайка.
 *
 * История: сначала пересчёта не было вовсе — merit-порядок был заморожен
 * между деплоями (аудит 2026-07-31, P1). Потом recomputeOne звался СИНХРОННО
 * в теле ответа — и читал ВСЕ лайки ВСЕХ фото автора на каждый клик: у
 * популярного автора это тысячи строк, дороже всего ровно в вирусный момент
 * (аудит 2026-09-10, П1). Теперь лайк лишь взводит needsRescore — одна
 * дешёвая запись; пересчёт коалесцируется 15-минутным джобом (rescoreMarked
 * в /api/jobs/inquiries): сто лайков серии = один recompute.
 * Ошибку глотаем: лайк пользователя не должен падать из-за пометки.
 */
async function recomputeAfterLike(profileId: string): Promise<void> {
  try {
    await db.photographerProfile.update({ where: { id: profileId }, data: { needsRescore: true } });
  } catch {
    // порядок обновится следующим плановым пересчётом — лайк уже сохранён
  }
}
